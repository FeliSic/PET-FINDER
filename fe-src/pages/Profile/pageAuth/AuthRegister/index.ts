...
class Register extends HTMLElement {
  ...
  async getUserLocation(): Promise<{ lat: number; lng: number; name: string }> {
      ...
      navigator.geolocation.getCurrentPosition(
        async (position) => {
          ...
          try {
            ...
            resolve({ lat, lng, name: `${lat.toFixed(6)}, ${lng.toFixed(6)}` });
          }
        },
        (error) => {
          reject(error);
  ...
  render() {
    ...
    const locationInfo = this.querySelector('.location-info') as HTMLDivElement;
    const locationText = this.querySelector('.location-text') as HTMLParagraphElement;
    ...
    locationBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        locationBtn.textContent = '✅ Ubicación guardada';
        locationBtn.style.backgroundColor = '#4caf50';
      } catch (error) {
        console.error('Error al obtener ubicación:', error);
        alert('No se pudo obtener tu ubicación. Podés continuar sin ella o intentar de nuevo.');
        locationBtn.textContent = '📍 Obtener mi ubicación';
    ...
    formUser.addEventListener("submit", async (e) => {
      ...
      const confirmPassword = (this.querySelector('.confirmPassword') as HTMLInputElement).value;
      const name = (this.querySelector('.nombre') as HTMLInputElement).value;
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error(result.error || 'Error al guardar el usuario');
        }

        console.log('Usuario guardado:', result);
        
        const userId = result["Usuario y Auth creados"]?.userId;
