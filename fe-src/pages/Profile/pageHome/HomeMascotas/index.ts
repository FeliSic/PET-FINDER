...
class PetsComponent extends HTMLElement {
  connectedCallback() {
    this.render();
  }

  async fetchNearbyPets(): Promise<{ pets: Pet[]; userLocation: any; radius: number }> {
  ...
  addReportListeners() {
    this.querySelectorAll('.report-button').forEach(btn => {
      btn.addEventListener('click', e => {
        const petId = (e.target as HTMLButtonElement).dataset.petId;
        const petName = (e.target as HTMLButtonElement).dataset.petName;
        if (petId) {
  ...
  showReportForm(petId: string, petName: string) {
    ...
    modal.querySelector('#report-form')!.addEventListener('submit', async e => {
      ...
      const reporterName = (modal.querySelector('#reporterName') as HTMLInputElement).value;
      const reporterPhone = (modal.querySelector('#reporterPhone') as HTMLInputElement).value;
      const location = (modal.querySelector('#location') as HTMLInputElement).value;
      const message = (modal.querySelector('#message') as HTMLTextAreaElement).value;
      ...
      try {
        ...
        alert('✅ ¡Reporte enviado con éxito! El dueño recibirá un email con tu información.');
        modal.remove();
      } catch (error: any) {
  ...
  async render() {
    ...
    try {
      ...
      this.innerHTML = `
        <div style="padding: 80px 20px 20px;">
          ...
          <p style="text-align: center; color: #666; margin-bottom: 30px;">
            Encontramos ${pets.length} mascota${pets.length > 1 ? 's' : ''} en un radio de ${radius}km
          </p>
              ...
              return `
                <div class="pet-card" style="border: 1px solid #ddd; border-radius: 10px; overflow: hidden; box-shadow: 0 2px 8px rgba(0,0,0,0.1); transition: transform 0.2s;">
                  ...
                  <div style="padding: 15px;">
                    ...
                    <p style="color: #666; margin: 5px 0;"><strong>📍</strong> ${pet.location}</p>
      ...
      const retryButton = this.querySelector('.retry-button') as HTMLButtonElement;
...
customElements.define('pets-component', PetsComponent);